...
class App extends Component {
  ...
  loadBooks = () =>{
    Repository.getBooks().then((data)=>{
      this.setState({
        books:data.data
      })
    }).then(()=>{
      Repository.getAuthors().then((data)=>{
        this.setState({
          authors:data.data
        })
      })
  ...
  onReturn = (id) =>{
    Repository.returnBook(id).then(()=>{this.loadBooks()});
  ...
  editBook=(id, name, category, author, available)=>{
    Repository.editBook(id,name,category,author,available).then(()=>{this.loadBooks()})
  }
